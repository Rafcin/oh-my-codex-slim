...
![OMCS performance evaluation comparing verified success, median wall time, and median tokens against Plain Codex](docs/assets/omcs-benchmark-results.svg)

The first valid public matrix completed on August 27, 2026: six tasks, three repetitions, and 18 matched outcomes per arm using `gpt-5.6-terra` at `high` reasoning. Plain Codex passed 16/18 hidden graders (88.9%, Wilson 95% CI 67.2–96.9%); OMCS auto passed 15/18 (83.3%, CI 60.8–94.2%). The paired delta is **−5.6 percentage points**, with a 100,000-resample paired-bootstrap 95% interval of **−22.2 to +11.1 points**. Discordant outcomes were 1 improvement and 2 regressions; the exact two-sided McNemar/binomial result is **p = 1.000**. This small matrix does not demonstrate an overall OMCS quality gain.

OMCS's median wall time was 129.0 seconds versus 64.9 seconds for Plain Codex: **1.99× slower** (paired-bootstrap 95% CI 1.80–2.49×). Median total tokens were 280.0k versus 87.4k. On the 17 complete usage pairs, the median-token ratio was **3.25×** (95% CI 2.63–4.47×). One OMCS diagnosis run timed out after 56 minutes and has no complete usage event; it remains a failed success/time outcome and is not imputed into token statistics. Both arms recorded zero safety violations.

![Verified hidden-grader outcomes for Plain Codex and OMCS across all six benchmark tasks](docs/assets/omcs-benchmark-task-outcomes.svg)

| Task | Plain Codex | OMCS auto | Result |
| --- | ---: | ---: | --- |
| Query parser | 3/3 | 3/3 | quality tie; OMCS 2.68× slower at median |
| Inventory reservation | 3/3 | 3/3 | quality tie; OMCS 2.36× slower |
| Artifact-path security | 3/3 | 3/3 | quality tie; OMCS 2.11× slower |
| Configuration diagnosis | 3/3 | 2/3 | OMCS regression; one timeout |
...
The public pilot includes reviewable reference solutions so the graders themselves can be audited. The following implementation was generated by OMCS in a valid passing treatment run:
...
It added focused tests for decoding, repeated keys, missing values, and pollution keys, and passed the hidden grader. Plain Codex also passed all three query-parser repetitions, so this demonstrates acceptable generated code—not superiority.
