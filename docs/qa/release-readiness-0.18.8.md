...
- No local `npm publish` command was run. The historical upstream workflow used a repository publishing credential; that inherited workflow is not part of OMCS.
...
- A historical upstream fallback verified and published `v0.18.8` without provenance after Fulcio failures. OMCS does not inherit that credentialed publishing path.
