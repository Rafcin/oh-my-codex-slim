...
Commands were run from `$UPSTREAM_WORKSPACE`; release gates that can inherit session/runtime variables were rerun with `USE_OMX_EXPLORE_CMD`, `OMX_ROOT`, `OMX_STATE_ROOT`, `OMX_TEAM_STATE_ROOT`, `OMX_SESSION_ID`, and `CODEX_SESSION_ID` unset where relevant.
