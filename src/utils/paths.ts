/**
 * Path utilities for oh-my-codex-slim
...
/** oh-my-codex-slim state directory (.omx/state/) */
export function omxStateDir(projectRoot?: string): string {
  return join(omxRoot(projectRoot), "state");
}

/** oh-my-codex-slim project memory file (.omx/project-memory.json) */
...
/** oh-my-codex-slim notepad file (.omx/notepad.md) */
export function omxNotepadPath(projectRoot?: string): string {
  return join(omxRoot(projectRoot), "notepad.md");
}

/** oh-my-codex-slim wiki directory (repository-root omx_wiki/) */
