...
describe("release verification isolation", () => {
	...
	it("uses only the repository validator and never spreads the host environment", async () => {
		const source = await readFile(
			join(process.cwd(), "src", "scripts", "verify-release.ts"),
			"utf8",
		);
		assert.doesNotMatch(
			source,
			/\.\.\.process\.env|homedir\(\)|\.codex.*validate_plugin/,
		);
		assert.match(
			source,
			/buildReleaseEnvironment\(\s*process\.env,\s*isolatedRoot,\s*process\.execPath,?\s*\)/,
		);
		...
		assert.match(
			source,
			/preserveNpmPackageArtifact\(\s*artifactPath,\s*approvedArtifactPath,\s*artifact,?\s*\)/,
		);
	...
	it("packs the public OMCS runtime, policy, provenance, guides, diagrams, and sanitized terminal views", () => {
		...
		const paths = new Set(
			(
				JSON.parse(packed.stdout) as Array<{ files: Array<{ path: string }> }>
	...
	it("rejects any package-manifest omission or unexpected public file", () => {
		...
		assert.throws(
			() =>
				assertApprovedPackedManifest(
					paths.filter((path) => path !== "README.md"),
				),
			/missing README\.md/,
		);
		for (const unexpected of [
			"plugins/oh-my-codex-slim/extra.js",
			"docs/.envrc",
			"docs/superpowers/plan.md",
			"docs/assets/temp-render.svg",
			"dist/router/adapter.js",
			"private-key.pem",
			"extensionless-binary",
		])
			assert.throws(
				() => assertApprovedPackedManifest([...paths, unexpected]),
				new RegExp(
					`unexpected ${unexpected.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
	});
});
