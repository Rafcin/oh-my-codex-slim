...
describe("OMCS configuration commands", () => {
	it("configures project and global policy and exposes effective source paths without dumping config", async () => {
		...
		try {
			...
			assert.equal(shown.sources.project, join(cwd, "omcs.config.json"));
			assert.equal((await validateOmcsConfigFile(join(cwd, "omcs.config.json"))).profile, "auto");
	...
	it("keeps session configuration process-local and never writes a file", async () => {
		...
		try {
			...
			await assert.rejects(readFile(join(cwd, "omcs.config.json")), { code: "ENOENT" });
		} finally { await rm(root, { recursive: true, force: true }); }
	});
	...
	it("parses the documented CLI forms and emits stable JSON", async () => {
		...
		try {
			...
			assert.deepEqual(JSON.parse(configure.stdout), { scope: "project", action: "create", path: join(await realpath(cwd), "omcs.config.json"), bytes: JSON.parse(configure.stdout).bytes, effectiveProfile: "auto" });
